          ...
          <div class="col-md-12">
            <div class="card">
                <div class="card-header bg-gray">
                  <h3 class="card-title">{{__('Manage Product Categories')}}</h3>
                  <a type="button" class="btn btn-sm btn-primary float-right" data-toggle="modal" data-target="#addProductGroup">{{__('New Product Category')}} </a>
                              ...
                              <div class="modal-dialog modal-lg" role="document">
                                <div class="modal-content bg-light-gray">
                                  <div class="modal-header bg-gray">
                                    <h5 class="modal-title" id="editProductGroupLabel">{{__('Update Product Category')}}</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                      <span aria-hidden="true">&times;</span>
                                    </button>
                                  </div>
                                  <div class="modal-body">
                                    <form action="{{url('product/categories', $productGroup)}}" method="POST">
                                        @csrf
                                        @method('PUT')
                            ...
                            {{-- !SECTION ADD Currency modal ends here --}}
                                    ...
                                    <span id="delbtn{{@$productGroup->id}}"></span>
                                      ...
                                      <form id="delete-productgroup-{{@$productGroup->id}}"
                                          action="{{ url('product/categories/destroy', $productGroup->id) }}"
                                          method="POST">
                                          @method('DELETE')
                                          @csrf
                                      </form>
...
<div class="modal fade" id="addProductGroup" tabindex="-1" role="dialog" aria-labelledby="addProductGroupLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content  bg-light-gray">
        <div class="modal-header  bg-gray">
          <h5 class="modal-title" id="addProductGroupLabel">{{__('Add Product Category')}}</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <form action="{{url('product/categories/store')}}" method="POST">
              @csrf
              <div class="form-group">
                <label for="">{{__('Product Category')}}</label>
...
@include('crm.categories.index_js')
@endsection
